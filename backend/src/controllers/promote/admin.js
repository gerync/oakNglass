...
export default async function PromoteToAdminController(req, res) {
    ...
    try {
        ...
        const result = await client.query(`
            ...
            WHERE emailcodes.type = 'admin-invite'`
        );
        ...
        if (new Date(expiresat) < new Date()) {
            await code('admin-invite', userid);
        ...
        await client.query('UPDATE users SET role = $1 WHERE uuid = $2', ['admin', userid]);
        await client.query('DELETE FROM emailcodes WHERE userid = $1 AND type = $2', [userid, 'admin-invite']);
        await client.query('COMMIT');
        res.status(200).json({ message: 'Gratulálunk, sikeresen előléptél adminisztrátorrá!' });
    }
    catch (err) {
